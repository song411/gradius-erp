  ...
  useEffect(() => {
    ...
    if (editTarget) {
      ...
      const rows: ItemRow[] = (editTarget.estimate_items || []).map(item => ({
        ...
        work_time: item.spec || '', is_leader: item.is_leader,
        item_type: (item.item_type as ItemType) || '인력', spec: '',
...
function A4Preview({
  ...
  return (
    <>
            ...
            <td style={{ verticalAlign: 'top', width: '50%', paddingRight: '8px' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', border: '1.5px solid #374151' }}>
                <tbody>
                  ...
                  <TblRow2 l1="상호" v1={selectedInq?.company_name || '(업체명)'} l2="참조" v2={form.manager || ''} />
      ...
      {/* 합계금액 한글 — 계약확정안내 아래, 품목 테이블 위 */}
      <div style={{
        textAlign: 'center', margin: '14px 0',
        padding: '10px 0', fontSize: '13px', fontWeight: '700', color: '#1e3a5f',
        ...
        <thead>
          <tr style={{ backgroundColor: '#374151', color: '#fff' }}>
            {['품명', '시간/규격', '수량', '일수', '단가', '금액', '비고'].map((h, i) => (
              <th key={h} style={{ padding: '7px 6px', textAlign: i < 2 ? 'left' : 'right', fontWeight: '600', fontSize: '11px', border: '1px solid #4b5563' }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {staffItems.filter(r => r.role_name).map((row, idx) => {
            ...
            return (
              <tr key={row.key} style={{ backgroundColor: idx % 2 === 0 ? '#fff' : '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
          ...
          {supportItems.filter(r => r.role_name).map((row) => (
            <tr key={row.key} style={{ backgroundColor: '#e0f2fe', borderBottom: '1px solid #bae6fd' }}>
              ...
              <td style={{ padding: '7px 6px', border: '1px solid #bae6fd', textAlign: 'right', fontWeight: '600', color: '#0369a1' }}>-</td>
      ...
      {form.notes && (
        <div style={{ marginBottom: '14px', border: '1px solid #fde68a', borderRadius: '4px', backgroundColor: '#fffbeb', padding: '8px 12px', fontSize: '11px' }}>
          ...
          <div style={{ color: '#451a03', whiteSpace: 'pre-wrap', lineHeight: '1.7' }}>{form.notes}</div>
        </div>
      )}
