...
import SalesBoardGuide from './SalesBoardGuide'
...
const CHANGELOGS = [
  {
    version: 'v1.6.0', date: '2026.09.21', label: '최신',
    color: 'bg-green-100 text-green-700',
    items: [
      '영업 보드 신설 — 체결 전 현황을 한 화면에서',
      '구분 탭 (전체·견적 발송 전·체결 전·체결·미체결) + 승률 집계',
      '통화 기록 · 다음 할 일 · 미체결 사유 남기기',
      '캘린더형 보기 — 그날 확정 인원까지 같이 보여 인력 경합 확인',
      '정렬 고르기 (급한 순 · 행사일 임박순 · 금액 큰 순 등)',
      '견적 일괄 발송 표시 (발송일 직접 지정)',
      '견적 품목이 저장 후 사라지던 문제 수정 (조회 1000행 상한)',
      '가이드북에 영업 보드 장 추가',
    ],
  },
  {
    version: 'v1.5.0', date: '2026.05.25', label: '',
...
const FLOW_STEPS = [
  ...
  {
    emoji: '🎯', title: '후속 관리', menu: '영업 보드',
    desc: '견적을 보낸 뒤 답이 올 때까지가 제일 잘 새는 구간입니다. 여기서 챙깁니다.',
    tips: ['통화하면 접촉 이력에 한 줄 남기기', '다음 할 일 + 날짜 넣기 → 그 날 오늘의 후속에 뜸', '14일 무응답이면 빨간불 — 확인 전화', '안 되면 미체결 + 사유까지 남기기'],
...
const MENUS = [
  ...
  {
    emoji: '🎯', name: '영업 보드', path: '/pipeline',
    desc: '체결 전 현황 — 견적 발송·답변 대기·다음 할 일을 한 화면에서.',
    buttons: [
      { name: '구분 탭 (전체/견적 발송 전/체결 전/체결/미체결)', desc: '탭을 누르면 그 묶음만 넓게 펼쳐집니다. 평소엔 "체결 전"에 가장 많이 머뭅니다' },
      { name: '오늘의 후속 (맨 위 파란 줄)', desc: '오늘까지 하기로 한 일. 기한이 지나면 빨갛게 표시됩니다' },
      { name: '카드 클릭', desc: '다음 할 일 · 접촉 이력 · 문의 원문 · 견적 목록 · 결론 처리를 한 창에서' },
      { name: '전화번호', desc: '카드에서 번호를 누르면 바로 전화가 걸립니다 (휴대폰)' },
      { name: '정렬', desc: '급한 순 / 행사일 임박순 / 오래 멈춘 순 / 금액 큰 순 / 최근 접수순' },
      { name: '보드 ↔ 캘린더', desc: '캘린더는 그날 이미 확정된 인원까지 같이 보여줍니다 (인력 경합 확인)' },
      { name: '간략히 / 자세히', desc: '카드에 적어둔 내용까지 펼칠지 고릅니다' },
    ],
    notes: [
      '신호등: 7일 노랑 · 14일 빨강 (무응답 기준). 행사일이 지난 건도 빨강',
      '미체결로 닫을 때 사유를 꼭 고르세요 — 다음 견적의 근거가 됩니다',
      '여기 보이는 금액은 아직 남의 돈입니다. 매출·수익 집계에는 들어가지 않습니다',
      '자세한 이유는 가이드북 "영업 보드" 장을 보세요',
    ],
  },
  {
...
const FAQS = [
  ...
  { q: '견적 발송 상태를 바꾸려면?', a: '견적관리 목록에서 발송 상태 배지(미발송/발송완료)를 직접 클릭하면 토글됩니다. 발송완료로 바꾸면 "발송 완료" 탭으로 자동 이동합니다. 여러 건은 목록 위 체크박스로 골라 한 번에 처리할 수 있습니다.' },
  { q: '견적을 여러 건 한꺼번에 발송완료로 바꾸려면?', a: '견적관리 목록 위 회색 바에서 체크박스로 고른 뒤, 발송일을 넣고 "발송완료로 표시"를 누르세요. 발송일은 반드시 실제로 보낸 날로 넣어야 합니다 — 오늘로 두면 영업 보드의 무응답 일수가 전부 틀어집니다.' },
  { q: '영업 보드에 왜 기록을 남겨야 하나요?', a: '통화 내용을 안 남기면 같은 질문을 두 번 하게 되고, 다음 할 일에 날짜를 안 넣으면 그 약속은 아무 데도 뜨지 않습니다. 미체결 사유를 안 적으면 왜 졌는지 영영 알 수 없습니다. 가이드북 "영업 보드" 장에 이유가 자세히 있습니다.' },
  { q: '영업 보드 카드가 빨간불인데 왜 그런가요?', a: '견적을 보낸 지 14일이 지나도록 답이 없거나, 행사일이 이미 지났는데 아직 체결 전인 건입니다. 확인 전화를 하거나, 끝난 건이면 미체결 + 사유를 남겨 정리하세요.' },
  { q: '"(추정)"이라고 붙은 건 뭔가요?', a: '옛날 데이터라 실제 발송일이 비어 있어서, 견적을 마지막으로 손본 날로 대신 센 것입니다. 앞으로 발송하는 건은 정확히 기록되니 시간이 지나면 사라집니다.' },
  { q: '체결된 건이 왜 영업 보드에도 보이나요?', a: '최근 30일에 체결된 건은 성과로 보여줍니다. 실패만 쌓이는 화면은 아무도 보지 않으니까요. 다만 체결 금액은 "진행 중 견적" 합계에 섞이지 않습니다.' },
]

type Section = 'flow' | 'sales' | 'scenarios' | 'kakao' | 'menus' | 'faq' | 'changelog'

const SECTIONS: { id: Section; label: string; emoji: string }[] = [
  { id: 'flow',       label: '업무 플로우',    emoji: '🔄' },
  { id: 'sales',      label: '영업 보드',      emoji: '🎯' },
